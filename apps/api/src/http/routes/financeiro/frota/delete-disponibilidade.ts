...
export async function deleteDisponibilidade(app: FastifyInstance) {
  app
    ...
    .delete(
      '/organizations/:slug/financeiro/frota/veiculos/:veiculoId/disponibilidades/:disponibilidadeId',
      {
        schema: {
          ...
          summary: 'Excluir disponibilidade do veículo',
          security: [{ bearerAuth: [] }],
          params: z.object({
            slug: z.string(),
      ...
      async (request, reply) => {
        ...
        if (cannot('delete', 'Lancamento')) {
          throw new UnauthorizedError(
            'Você não tem permissão para excluir disponibilidade.',
          )
        }

        await FrotaService.excluirDisponibilidade(
          organization.id,
          veiculoId,
          disponibilidadeId,
        )
