...
export async function putAbastecimento(app: FastifyInstance) {
  app
    ...
    .put(
      '/organizations/:slug/financeiro/frota/veiculos/:veiculoId/abastecimentos/:abastecimentoId',
      {
        schema: {
          ...
          summary: 'Atualizar abastecimento e lançamento vinculado',
          security: [{ bearerAuth: [] }],
          params: z.object({
            slug: z.string(),
      ...
      async (request, reply) => {
        ...
        if (cannot('update', 'Lancamento')) {
          throw new UnauthorizedError('Você não tem permissão para editar abastecimentos.')
        }

        const b = request.body
        await FrotaService.atualizarAbastecimento(organization.id, veiculoId, abastecimentoId, {
