  ...
  app
    ...
    .get(
      ...
      async (request, reply) => {
        ...
        const { tipo, ativo, search, page, limit, orderBy, order } = request.query
        ...
        if (cannot('get', 'CategoriaFinanceira')) {
          throw new UnauthorizedError('Você não tem permissão para visualizar categorias.')
        }
        ...
        return reply.send({ categorias, total })
      },
    )
}