#!/usr/bin/env sh
set -eu
echo "oc-worker: aguardando Redis em ${REDIS_URL:-?}..."
ATTEMPT=1
...
while [ "$ATTEMPT" -le "$MAX" ]; do
  if node -e "const Redis=require('ioredis'); const r=new Redis(process.env.REDIS_URL,{maxRetriesPerRequest:1,connectTimeout:2000}); r.ping().then(()=>{console.log('ok'); r.quit(); process.exit(0)}).catch(()=>process.exit(1))" 2>/dev/null; then
    echo "oc-worker: Redis OK (tentativa ${ATTEMPT})"
    break
  fi
  echo "oc-worker: Redis indisponível — tentativa ${ATTEMPT}/${MAX}"
  ATTEMPT=$((ATTEMPT + 1))
  sleep 2
done
if [ "$ATTEMPT" -gt "$MAX" ]; then
  echo "oc-worker: ERRO — Redis não respondeu." >&2
  exit 1
fi
exec node /app/apps/api/dist/oc-worker.js
